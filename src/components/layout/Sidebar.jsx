    ...
    <aside className="sidebar fixed top-0 left-0 h-full w-[280px] z-40 hidden md:flex flex-col">
      ...
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-white">Gestor Finanzas</p>
            <p className="text-xs text-slate-400">Balance total: {formatCurrency(totalBalance, settings.main_currency)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-white/10">
        <div className="flex items-center gap-3 mb-3">
          {user?.user_metadata?.avatar_url
            ? <img src={user.user_metadata.avatar_url} alt="avatar" className="w-9 h-9 rounded-full border border-white/20" />
            : <div className="w-9 h-9 rounded-full bg-blue-500/30 flex items-center justify-center text-blue-300 font-bold">{user?.email?.[0]?.toUpperCase()}</div>
          }
        ...
        <button onClick={signOut} className="w-full btn-neutral py-2 text-sm flex items-center justify-center gap-2">
